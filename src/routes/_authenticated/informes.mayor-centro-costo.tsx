...
function Body({ companyId, from, to, costCenterId }: { companyId?: string; from: string; to: string; costCenterId: string | null }) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // 1. Limpieza básica de fechas para asegurar formato YYYY-MM-DD
      const cleanFrom = from.split("T")[0];
      const cleanTo = to.split("T")[0];

      // 2. Query con relación interna (!inner) filtrando company_id en el join
      let q = supabase
        .from("journal_lines")
        .select(`
          debit,
          ...
          cc:cost_centers(code, name),
          entry:journal_entries!inner(entry_date, entry_number, company_id, status)
        `)
        .eq("entry.company_id", companyId!)
        .neq("entry.status", "anulado")
        .gte("entry.entry_date", cleanFrom)
        .lte("entry.entry_date", cleanTo);

      if (costCenterId) {
        q = q.eq("cost_center_id", costCenterId);
      } else {
        q = q.not("cost_center_id", "is", null);
      }

      const { data, error } = await q;

      if (error) {
        console.error("Error cargando mayor por CC:", error);
        throw error;
      }
  ...
  if (grouped.size === 0) {
    return <div className="text-center py-8 text-muted-foreground">Sin movimientos con centro de costo.</div>;
  }
        ...
        return (
          <div key={g.code} className="border rounded-md">
            <div className="bg-muted px-3 py-2 font-medium text-sm">
              <span className="font-mono mr-2">{g.code}</span>{g.name}
            </div>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  return (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-sm">
                        <span className="font-mono text-xs mr-1">{l.account?.code}</span>{l.account?.name}
                      </TableCell>
